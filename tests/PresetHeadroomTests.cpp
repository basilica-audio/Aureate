...
namespace
{
    ...
    // What a shipped `output` trim would AIM at, 0.3 dB below the asserted
    // line. The gap is deliberate: it is the difference between "an upstream
    // voicing tweak moved the peak a little" and "this gate goes red". A trim
    // is derived as (measured overshoot + 0.3 dB), rounded up to the output
    // parameter's step - never tasted, and never applied to a preset that was
    // already under the line. No preset in this repo currently needs one.
    constexpr double headroomTargetDbfs = -0.3;

    // Fourteen factory presets ship - CMakeLists.txt's juce_add_binary_data
    ...
    constexpr int shippedFactoryPresetCount = 14;
    ...
    enum class Arrival
    {
        ...
        // Mid-session recall: the plugin is already prepared and running at some
        // other setting when the user clicks the preset in the browser, so every
        // parameter JUMPS from where it was to where the preset puts it while
        // the DSP is primed for the old values. This measures the preset plus
        // whatever the transition into it costs - which is the thing a user
        // hears in the first ten seconds, and which found a 17.6 dB defect in
        // this plugin's Iron stage (see src/dsp/IronStage.h's
        // rescaleAdaaHistoryForDriveChange()).
        midSessionRecall
    };
    ...
    // the full band this plugin shapes, from the Iron stage's 35 Hz core bump to
    // its ~19 kHz leakage roll-off, rather than only the middle of it.
    //
    // Aureate is a bus/glue processor and would in principle be better
    // characterised by a dense mix than by a single instrument. The fleet
    // reference is used anyway, deliberately: what this gate measures is a
    // PEAK against full scale, a dense mix is not more demanding than a plucked
    // transient at the same peak level (it is less so, being more sustained),
    // and using one signal across all thirteen plugins is what makes their
    // headroom numbers comparable at all. A domain-specific reference would buy
                                      ...
                                      Arrival arrival)
    {
        ...
        if (arrival == Arrival::sessionLoad)
        ...
        if (arrival == Arrival::midSessionRecall)
            REQUIRE (processor.presetManager.loadPreset (presetName));
...
TEST_CASE ("Factory presets: none of them push the reference programme past 0 dBFS", "[presets][headroom]")
{
    ...
    for (const auto& entry : presets)
    {
        ...
        // Both ways a user arrives at the preset, because they are not the same
        // measurement (see Arrival). A preset is only safe if it is safe both
        // as a restored session and as a click in the preset browser.
        for (const auto arrival : { Arrival::sessionLoad, Arrival::midSessionRecall })
        {
            INFO ("arrival: " << (arrival == Arrival::sessionLoad ? "session load"
                                                                  : "mid-session recall"));

            const auto peakDb = renderFactoryPresetPeakDb (entry.name, input, arrival);
...
// of every factory preset on both arrival paths, which is how a trim would be
// derived if one were ever needed.
TEST_CASE ("Factory preset headroom table", "[.headroom-table]")
{
    ...
    for (const auto& entry : probe.presetManager.getAllPresets())
    {
        ...
        const auto worstDb = juce::jmax (loadDb, recallDb);

        WARN (entry.name << " | peak " << worstDb << " dBFS (session load " << loadDb
              << ", recall " << recallDb << ") | trim to hit " << headroomTargetDbfs
              << " dBFS: " << (headroomTargetDbfs - worstDb) << " dB");
    }
}
